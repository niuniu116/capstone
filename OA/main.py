...
from model.CNN_gate_attention256_VIT import CNN_TFV_CrossAttentiongate
from model.CNN_gate_attention512_VIT import CNN_TFV_Gate512
from model.CNN_gate_attention import CNN_Gate_Multiview
from model.VIT_gate_attention import ViT_GateAttention
...
def set_args():
    ...
    # use_vit; vgg_vit_exchange; vgg_vit_fusion; cnn; vit_token_fusion;
    # CNN_gate_attention256_VIT; CNN_gate_attention512_VIT; cnn_tfv_crossattention(Can only perform time-series tasks);
    # CNN_gate_attention; VIT_gate_attention;
    # cnn_lstm(second model, only perform time-series tasks); cnn_lstm_attention(best model, only perform time-series tasks)
    ...
    # Para for visualization
    parser.add_argument('--visualize',                      type=bool, default=False, help='Whether to enable Grad-CAM visualization')
    parser.add_argument('--visualize_interval',             type=int, default=1, help='How many epochs to visualize once')
    parser.add_argument('--visualize_output_dir',           type=str, default='vis_results', help='Grad-CAM visualization result output path')
...
if __name__ == '__main__':
    ...
    print(f"[INFO] This run uses seed: {seed}")
    set_seed(seed)
    ...
    if args.visual_TFV:
        ...
        elif args.model_type == 'CNN_gate_attention256_VIT':
            model = CNN_TFV_CrossAttentiongate(args)
        elif args.model_type == 'CNN_gate_attention512_VIT':
            model = CNN_TFV_Gate512(args)
        elif args.model_type == 'CNN_gate_attention':
            model = CNN_Gate_Multiview(args)
        elif args.model_type == 'VIT_gate_attention':
