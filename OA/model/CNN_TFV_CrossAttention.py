...
# ## this class is for 'attention' fusion function.
# class AttentionFusion(nn.Module):
#     def __init__(self, input_dim):
#         super().__init__()
#         self.attention = nn.Sequential(
#             nn.Linear(input_dim, 128),   # 每帧是 C 维输入
#             nn.ReLU(),
#             nn.Linear(128, 1)            # 输出一个标量权重
#         )
#         self.softmax = nn.Softmax(dim=1)
#
#     def forward(self, v00, v12, v24):
#         x = torch.stack([v00, v12, v24], dim=1)  # [B, 3, C]
#
#         # 对每一帧分别计算 attention 权重
#         attn_weights = self.attention(x)         # [B, 3, 1]
#         attn_weights = self.softmax(attn_weights)  # [B, 3, 1]
#
#         # print("Attention Weights (per batch sample):")
#         # print(attn_weights.squeeze(-1).detach().cpu().numpy())
#
#         fusion = torch.sum(attn_weights * x, dim=1)  # 加权求和 → [B, C]
#         return fusion
